...
// struct.rs
//
// Dans cet exercice on va écrire notre première structure et ses méthodes,
// fonctions associées, dans un second temps on verra comment faire du passage
// par référence
//
...
// - self
// - passage par référence(borrowing), copie et sémantiques de mouvement (move)
//
...
// 1. On ajoute une fonction `new` pour créer un Point2D
...
impl Point2D { // Le type Self vaut le type après `impl`
    ...
    /// Offre une représentation textuelle pour un humain d'un point
    /// `Point2D { x: 3, y: 5 }
    fn show(self) -> String {
...
fn main() {}
// Pour commencer à écrire les tests il faudra dé-commenter le bloc ci-dessous :
#[cfg(test)]
mod tests {
    use super::*;
